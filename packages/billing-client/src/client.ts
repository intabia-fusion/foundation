...
export class BillingClient {
  ...
  // Reset the AI usage window for a workspace (one-time purchase effect). Idempotent by purchaseId.
  async resetAiWindow (workspace: WorkspaceUuid, purchaseId: string, resetAtMs?: number): Promise<{ applied: boolean }> {
    const path = `/api/v1/${workspace}/ai/tokens/reset`
    const url = new URL(concatLink(this.endpoint, path))
    const body = JSON.stringify({ purchaseId, resetAtMs })
