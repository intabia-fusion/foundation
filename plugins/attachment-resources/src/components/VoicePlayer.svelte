...
<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      void ctx.close()
      // webm/opus blobs from MediaRecorder report duration=Infinity on the <audio> element until a
      // full seek, so the player looks like it is still recording. The decoded buffer has the real
      // length - use it (bind:duration dropped so the element can't overwrite it with Infinity).
...
<div class="player">
  ...
  <a class="dl" bind:this={downloadEl} href={src} download={name} on:click|stopPropagation>
    <ButtonIcon icon={FileDownload} size="small" kind="tertiary" on:click={() => downloadEl.click()} />
  </a>
</div>
