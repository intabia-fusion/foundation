...
**`pnpm svelte-check` глотает вывод.** Показывает `FAILURE: <pkg>` с пустым
телом, и легко принять за предсуществующий шум - я так и сделал, и пропустил
три реальные регрессии. Гонять надо в самом пакете: `cd <pkg> && pnpm run
...
подменённые. После `-X ours` обязателен `pnpm run svelte-check` в каждом
