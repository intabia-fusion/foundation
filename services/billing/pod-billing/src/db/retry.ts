...
export class RetryDB implements BillingDB {
  ...
  async getProviderPool (ctx: MeasureContext, providerId: string, model: string): Promise<ProviderPool | undefined> {
    return await retry(() => this.db.getProviderPool(ctx, providerId, model), this.options)
  }
  ...
  async upsertTokenBalance (
    ...
    await retry(() => this.db.upsertTokenBalance(ctx, workspace, remainingTokens, periodStart), this.options)
  }

  async getAiWindowReset (ctx: MeasureContext, workspace: WorkspaceUuid): Promise<number | undefined> {
    return await retry(() => this.db.getAiWindowReset(ctx, workspace), this.options)
  }
  ...
  async setAiWindowReset (
    ...
    return await retry(() => this.db.setAiWindowReset(ctx, workspace, resetAtMs, purchaseId), this.options)
  }
}
