...
Проверка «меня перенесли из моей комнаты» в `ControlExt.svelte` читала внешнюю `myRoomAttached` -
её нет в зависимостях реактивного выражения, выполнялась на такт раньше пересчёта. Вторая вкладка
через 50 мс после коннекта звала `leaveMeeting()`. Детали и воспроизведение -
`docs/memory/sanity-flaky-tests.md`, раздел «love multitab».
