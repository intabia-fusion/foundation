...
<script lang="ts">
  ...
  async function removeAttachment (attachment: Attachment): Promise<void> {
    ...
    attachments = attachments
    // Write-through: already in DB, delete now instead of deferring to send.
    // deleteAttachment() branches on originalAttachments membership (removeCollection vs deleteFile),
    // so it MUST run before we drop the id from the sets - else a persisted doc only loses its blob
    // and the orphan re-attaches to the message on send.
