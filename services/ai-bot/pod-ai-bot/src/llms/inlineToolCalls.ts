...
// Local OpenAI-compatible servers without a tool-call parser (gpt-oss / GigaChat
// harmony format) leak the call into the assistant text, e.g.:
//   <|function_call|>{"name":"update_user_memory","arguments":{...}}
// Match the marker followed by a JSON object; tolerate a trailing channel/end token.
...
/**
 * Parse inline `<|function_call|>{...}` markers out of model text into structured
 * ToolCalls. Returns the recovered calls plus the leftover human-facing content
 * (markers + their JSON removed). If no markers are present, content is returned
 * unchanged and toolCalls is empty.
 */
