...
<script lang="ts">
  ...
  // Stop the mic (releases the browser recording indicator), upload the blob, create the
  // AudioTranscribe doc attached to the draft and register it as a draft attachment so it previews
  // and is owned by the message on send. Returns the created doc, or undefined on failure.
  async function persist (): Promise<AudioTranscribe | undefined> {
    stopTimers()
    let rec
    try {
      // denoise off: its noise profile eats the first frames, clipping speech that starts
      // immediately. Re-enable once it estimates noise from a trailing/quiet window instead.
      rec = await recorder.stop(false)
    } catch {
      dispatch('close')
      return undefined
    }
...
<div class="hud">
  {#if phase === 'starting'}
    ...
    <ButtonIcon icon={IconClose} size="small" kind="tertiary" tooltip={{ label: chunter.string.VoiceCancel }} on:click={onCancel} />
  {:else if phase === 'recording'}
    ...
    <ButtonIcon icon={IconClose} size="small" kind="tertiary" tooltip={{ label: chunter.string.VoiceCancel }} on:click={onCancel} />
    <ButtonIcon icon={IconCheck} size="small" kind="secondary" tooltip={{ label: chunter.string.VoiceAttach }} on:click={onAttach} />
    <ButtonIcon icon={IconSend} size="small" kind="primary" tooltip={{ label: chunter.string.VoiceSend }} on:click={onSend} />
  {:else}
    <Spinner size="small" />
