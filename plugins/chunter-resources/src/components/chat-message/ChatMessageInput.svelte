...
<script lang="ts">
  ...
  // The HUD created the AudioTranscribe in the DB (attachedTo = this draft's _id); register it as a
  // draft attachment so it previews, is removable, and is owned by the message on send. The
  // transcript stays on the attachment (editable inplace); it is not injected into the input.
